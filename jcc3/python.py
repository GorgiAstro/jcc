...
def compile(env, jccPath, output, moduleName, install, dist, debug, jars,
    ...
    if find_jvm_dll:
        line(out, 0, "import sys")
        line(out, 0, "if sys.platform == 'win32':")
        line(out, 1, "from .windows import add_jvm_dll_directory_to_path")
        line(out, 1, "add_jvm_dll_directory_to_path('%s')" %(find_jvm_dll))
        modules.append(os.path.join(
            os.path.dirname(os.path.realpath(__file__)), 'windows.py'))
    ...
    if resources:
        def copytree(src, dst):
            ...
            if not os.path.exists(_dst):
                print(f"creating directory for resource {dst}")
                os.mkdir(_dst)
            ...
            if os.path.isdir(module):
                def copytree(src, dst, is_package):
                    ...
                    if not os.path.exists(dst):
                        print(f"creating directory for module {dst}")
                        os.mkdir(dst)
                    for name in os.listdir(src):
                        if name.startswith('.'):
                            continue
    ...
    if shared:
        def copytree(src, dst):
            _dst = os.path.join(modulePath, dst)
            if not os.path.exists(_dst):
                print(f"creating directory for shared {dst}")
                os.mkdir(_dst)
            for name in os.listdir(src):
                if name.startswith('.'):
                    continue
