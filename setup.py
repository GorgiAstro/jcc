...
# separator.
#
# On macos (darwin), the JAVA_HOME environment variable may be used to
# override the /usr/libexec/java_home program.
...
INCLUDES = {
    ...
    'darwin/temurin': ['%(darwin)s/include' %(JDK),
                       '%(darwin)s/include/darwin' %(JDK)],
    'darwin/adoptopenjdk': ['%(darwin)s/include' %(JDK),
                            '%(darwin)s/include/darwin' %(JDK)],
...
LFLAGS = {
    ...
    'darwin/temurin': ['-L%(darwin)s/lib' %(JDK), '-ljava',
                       '-L%(darwin)s/lib/server' %(JDK), '-ljvm',
                       '-Wl,-rpath', '-Wl,%(darwin)s/lib' %(JDK),
                       '-Wl,-rpath', '-Wl,%(darwin)s/lib/server' %(JDK),
                       '-mmacosx-version-min=11.6'],
    'darwin/adoptopenjdk': ['-L%(darwin)s/lib' %(JDK), '-ljava',
                            '-L%(darwin)s/lib/server' %(JDK), '-ljvm',
                            '-Wl,-rpath', '-Wl,%(darwin)s/lib' %(JDK),
                            '-Wl,-rpath', '-Wl,%(darwin)s/lib/server' %(JDK),
                            '-mmacosx-version-min=10.9'],
    'darwin/home': ['-L%(darwin)s/lib' %(JDK), '-ljava',
                    '-L%(darwin)s/lib/server' %(JDK), '-ljvm',
                    '-Wl,-rpath', '-Wl,%(darwin)s/lib' %(JDK),
                    '-Wl,-rpath', '-Wl,%(darwin)s/lib/server' %(JDK),
                    '-mmacosx-version-min=10.9'],
